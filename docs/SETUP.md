...
4. **App roles** (recommandé, plutôt que `ADMIN_EMAILS`/`COMPLIANCE_EMAILS`) :
   créer `admin` et `compliance` (*Allowed member types* : Users/Groups), puis
   les assigner à des groupes dans **Enterprise applications → Users and
   groups**. L'orchestrator dérive le RBAC de ces rôles
...
2. **Authentication → Add a platform → Web**, redirect URI
   `https://admin.oao.northbridge.example/api/auth/callback/microsoft-entra-id`
   (convention Auth.js).
3. **Certificates & secrets → New client secret** →
   `AUTH_MICROSOFT_ENTRA_ID_SECRET`.
4. Générer la clé de signature de session : `openssl rand -base64 32` →
   `AUTH_SECRET`.
5. Restreindre l'accès : **Enterprise applications → Properties →
   Assignment required = Yes**, puis assigner uniquement les groupes
   `admin`/`compliance`.
6. Valeurs du chart : `admin.entra.clientId`, `admin.entra.tenantId`
   (par défaut celui de `auth.aad.tenantId`), `admin.authMode: aad`.
