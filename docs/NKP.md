...
| Deployment `oao-worker` | orchestrator `ROLE=all` + `WORKERS_ENABLED=true` (sync Graph, précalcul, daily brief, rétention) — hors Ingress, joignable seulement par le kubelet et le monitoring | 1 réplique, 500m/1Gi, `strategy: Recreate` |
...
> Cette section vérifie que **le déploiement** est sain. La checklist de
> **mise en production** (Entra ID, secrets, endpoint IA validé, politique
> Exchange, sauvegardes testées, alertes routées, pilote, rollback) est
