...
# Smart Alerts: open = an on-send check that itself fails does not block the
# send (default); closed = the send is refused until the check works again.
ARG VITE_ONSEND_FAIL_MODE=open
ARG VITE_COMPLIANCE_EMAIL="compliance@northbridge.example"
# The typed client concatenates `Routes.*`, which already start with
# /api/v1 (packages/shared API_PREFIX): VITE_API_BASE_URL is the bare ORIGIN.
ENV VITE_API_BASE_URL=https://$API_HOST \
