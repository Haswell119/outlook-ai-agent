...
| `llm.baseUrl` / `llm.model` | endpoint OpenAI-compatible interne (vLLM…) et `--served-model-name` |
...
| Rôle | Objet | Réplicas | Contenu |
|---|---|---|---|
| `ROLE=api` | Deployment `-api` + Service + HPA + PDB | 2 → 6 (HPA CPU/RPS) | API HTTP `/api/v1/*`, `/metrics` |
| `ROLE=worker` | Deployment `-worker` + Service | 1 (`strategy: Recreate`) | mailbox sync, précalcul, daily brief, rétention. Élection de leader par advisory lock PostgreSQL |

Le Service du worker n'est pas routé par l'ingress : il n'existe que pour le
scrape Prometheus (`oao_mailbox_sync_lag_seconds`, profondeur de file).
...
Contrat de métriques attendu de l'orchestrator (prom-client) :
`oao_http_requests_total`, `oao_http_request_duration_seconds`,
`oao_llm_calls_total{outcome}`, `oao_llm_call_duration_seconds`,
`oao_llm_circuit_open`, `oao_llm_queue_depth`, `oao_llm_queue_running`,
`oao_cache_hits_total{cache}`, `oao_cache_misses_total{cache}`,
`oao_mailbox_sync_lag_seconds`, `oao_mailbox_sync_runs_total{outcome}`,
`oao_audit_events_total{type}`, `oao_db_up`, `oao_build_info{version}`.
