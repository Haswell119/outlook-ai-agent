...
export function mockSystemStatus(): SystemStatus {
  return {
    ...
    sync: store().sync,
    // Structured-decision engine evaluated in shadow mode (the recommended first step).
    decisioning: {
      provider: "laya",
      mode: "shadow",
      state: "ok",
...
export function mockHealth(): Health {
  return {
    ...
    checks: {
      ...
      laya: { status: "ok", detail: "shadow · english, multilingual · cpu" },
    },
    version: "0.1.0",
