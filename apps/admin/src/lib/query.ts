...
export interface ResolvedQuery extends Partial<AuditQuery> {
  ...
  /**
   * `EmailAnalysis.source` as mirrored into `AuditEvent.details.source`, and the
   * model name. Not part of `AuditQuery` in the contract yet — see the README —
   * so they are forwarded as extra query parameters and applied exactly in mock
   * mode.
   */
  source?: AiSourceFilter;
  model?: string;
}

export const AI_SOURCE_FILTERS = ["llm", "cache", "precomputed", "heuristic"] as const;
export type AiSourceFilter = (typeof AI_SOURCE_FILTERS)[number];

/** Everything the data layer accepts: the contract query plus the two extras. */
export interface AuditQueryLike extends Partial<AuditQuery> {
  source?: AiSourceFilter;
  model?: string;
}
...
export function resolveQuery(params: SearchParamsInput): ResolvedQuery {
  ...
  if (source && (AI_SOURCE_FILTERS as readonly string[]).includes(source)) {
    query.source = source as AiSourceFilter;
  }
