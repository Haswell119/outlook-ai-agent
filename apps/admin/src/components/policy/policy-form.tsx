...
export function PolicyForm({
  ...
  /** Compiled once per keystroke: the inline error under each pattern row. */
  const patternChecks = React.useMemo(
    () => draft.sensitiveDataPatterns.map((p) => validateRegex(p.pattern)),
    [draft.sensitiveDataPatterns],
  );
  const hasInvalidPattern = patternChecks.some((c) => !c.valid);
  ...
  const save = async () => {
    ...
    // Every sensitive-data pattern must compile before it reaches the engine.
    const badPatterns = parsed.data.sensitiveDataPatterns
      .map((p) => ({ name: p.name || p.pattern, check: validateRegex(p.pattern) }))
      .filter((p) => !p.check.valid)
      .map((p) => `${p.name}: ${t("policy.regexInvalid")}${p.check.error ? ` — ${p.check.error}` : ""}`);
    ...
    try {
      ...
      // The orchestrator returns the stored version, with `updatedBy` / `updatedAt`.
      const next = (await res.json()) as Policy;
      setDraft(next);
      setSaved(true);
      toast({ title: t("policy.saved"), tone: "success" });
      router.refresh();
    } catch (e) {
      const message = e instanceof Error ? e.message : "Save failed";
      setErrors([message]);
      toast({ title: t("policy.saveFailed"), description: message, tone: "error" });
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={`${index}-${p.name}`}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      aria-invalid={patternChecks[index]?.valid === false}
                      aria-describedby={`pattern-check-${index}`}
                      className={
                        patternChecks[index]?.valid === false
                          ? "h-8 border-[#C4314B] font-mono text-xs"
                          : "h-8 font-mono text-xs"
                      }
                    />
                    <p
                      id={`pattern-check-${index}`}
                      className={
                        patternChecks[index]?.valid === false
                          ? "mt-1 text-xs font-medium text-[#C4314B]"
                          : "mt-1 text-xs text-[#107C10]"
                      }
                    >
                      {patternChecks[index]?.valid === false
                        ? `${t("policy.regexInvalid")}${patternChecks[index]?.error ? `: ${patternChecks[index]?.error}` : ""}`
                        : t("policy.regexValid")}
                    </p>
                  </TableCell>
                  <TableCell>
      ...
      {errors.length > 0 && (
        <ul
          role="alert"
          data-testid="policy-errors"
          className="rounded-md border border-[#C4314B]/30 bg-[#FDE7E9] p-3 text-xs text-[#C4314B]"
        >
          {errors.map((e) => (
            <li key={e}>{e}</li>
          ))}
      ...
      <PolicyTestPanel policy={draft} messages={messages} />

      <div className="flex flex-wrap items-center gap-3">
        <Button onClick={() => void save()} disabled={busy || hasInvalidPattern}>
        ...
        <span className="text-xs text-[#616161]" data-testid="policy-version">
          {t("policy.updatedBy")}: {draft.updatedBy ?? "—"}
          {draft.updatedAt &&
            ` · ${t("policy.updatedAt")}: ${formatDateTime(draft.updatedAt, language, timeZone)}`}
        </span>
      </div>
    </div>
  );
