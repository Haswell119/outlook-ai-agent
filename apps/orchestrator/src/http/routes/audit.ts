...
export async function auditRoutes(app: FastifyInstance, c: Container) {
  ...
  app.get(Routes.auditExport, { preHandler: requireRole("admin", "compliance") }, async (req, reply) => {
    ...
    const csv = await c.services.audit.exportCsv(req.user, filters);
    return reply.header("content-type", "text/csv; charset=utf-8").header("content-disposition", `attachment; filename="audit-${new Date().toISOString().slice(0, 10)}.csv"`).send(csv);
  });
