...
export class ActionsService {
  ...
  async propose(ctx: RequestContext, req: ProposeActionsRequest): Promise<ActionProposal> {
    ...
    const now = nowIso();
    const expiresAt = new Date(Date.parse(now) + PROPOSAL_TTL_MS).toISOString();
  ...
  /**
   * Approve and execute the selected actions.
   *
   * `Idempotency-Key` (optional header, kept for `IDEMPOTENCY_TTL_HOURS`):
   * a retry after a network timeout returns the stored response instead of
   * executing everything a second time — creating two calendar events or
   * escalating twice is exactly the kind of "AI did something odd" incident
   * this product must not produce. Replaying a key with a *different* body is
   * a `409 conflict`, never a silent wrong answer.
   */
  async approve(ctx: RequestContext, req: ApproveActionsRequest, opts: { idempotencyKey?: string } = {}): Promise<ApproveActionsResponse> {
    const { user } = ctx;
    const idemKey = opts.idempotencyKey?.trim();
    const requestHash = sha256(`approve\u0000${req.proposalId}\u0000${[...req.actionIds].sort().join(",")}\u0000${req.comment ?? ""}`);
    if (idemKey) {
      const existing = await this.deps.repos.idempotency.get(user.id, idemKey).catch(() => undefined);
      if (existing) {
        if (existing.requestHash !== requestHash) throw AppError.conflict(`Idempotency-Key ${idemKey} was already used with a different request body`);
        await this.audit.record({ user, type: "action_approved", approvalStatus: "approved", approvedBy: user.email, correlationId: ctx.correlationId, details: { proposalId: req.proposalId, idempotentReplay: true, idempotencyKey: idemKey } });
        return ApproveActionsResponseSchema.parse(existing.response);
      }
    }
    const response = await this.approveOnce(ctx, req);
    if (idemKey) {
      await this.deps.repos.idempotency
        .put({ key: idemKey, userId: user.id, requestHash, response, createdAt: nowIso(), expiresAt: new Date(Date.now() + this.deps.cfg.IDEMPOTENCY_TTL_HOURS * 3_600_000).toISOString() })
        .catch((e: Error) => this.deps.logger.warn({ err: e.message }, "could not store idempotency record"));
    }
    return response;
  }

  private async approveOnce(ctx: RequestContext, req: ApproveActionsRequest): Promise<ApproveActionsResponse> {
    const { user } = ctx;
