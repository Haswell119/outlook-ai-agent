...
import { ChatResponseSchema } from "@oao/shared";
...
export class ChatService {
  ...
  async chat(ctx: RequestContext, req: ChatRequest): Promise<ChatResponse> {
    ...
    if (req.currentEmail) {
      ...
      sources.push({ emailId: e.id, conversationId: e.conversationId, subject: e.subject, from: e.from?.name ?? e.from?.address, date: e.receivedAt ?? e.sentAt, relevance: 1, excerpt: bestExcerpt(e.body, req.message, 300), webLink: e.webLink });
    }
