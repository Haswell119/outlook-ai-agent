 ...
 */
const P = {
  jane: { name: "Jane Smith", address: "jane.smith@longbow.ch" },
  marc: { name: "Marc Dubois", address: "marc.dubois@longbow.ch" },
  sophie: { name: "Sophie Martin", address: "sophie.martin@longbow.ch" },
  compliance: { name: "Compliance Team", address: "compliance@longbow.ch" },
  legal: { name: "Legal Team", address: "legal@longbow.ch" },
  ...
  phisher: { name: "Longbow IT Support", address: "it-support@longbovv-finance.com" },
};
...
const specs: Spec[] = [
  ...
  { daysAgo: 16, from: "james", to: ["jane"], subject: "Mandate Approval Confirmation", conversationId: CONV, body: "Dear Jane,\n\nThis is to confirm that ABC Capital's investment committee has approved the discretionary mandate with Longbow Finance (Project Horizon). The signed Account Mandate is being couriered today and should reach you by Monday.\n\nBest regards,\nJames" },
  ...
  { daysAgo: 9, from: "jane", to: ["james"], cc: ["laura", "marc", "sophie"], subject: "Project Horizon – onboarding complete, welcome to Longbow", conversationId: CONV, body: "Dear James, dear Laura,\n\nWe are pleased to confirm that the onboarding of ABC Capital under Project Horizon is complete. The portfolio (reference HZ-2025-0417) is active as of today and the first investments will be executed according to the agreed balanced profile.\n\nThank you for your cooperation.\n\nKind regards,\nJane Smith" },
  { daysAgo: 8, from: "james", to: ["jane"], subject: "RE: Project Horizon – onboarding complete, welcome to Longbow", conversationId: CONV, body: "Thank you Jane – great news. Looking forward to the first monthly report.\n\nJames" },
  ...
  { daysAgo: 1, from: "phisher", to: ["jane"], subject: "URGENT: your mailbox password expires today", body: "Dear user,\n\nYour Longbow mailbox password expires today. To avoid losing access, verify your account immediately by logging in at http://185.203.116.42/owa/login and confirm your password.\n\nThis is your final notice.\n\nIT Support" },
...
export function sampleEmails(now = new Date()): EmailContext[] {
  return specs.map((s, i) => {
    ...
    return {
      ...
      internetMessageId: `<${id}@demo.longbow.ch>`,
      subject: s.subject,
      from: P[s.from],
      to: s.to.map((k) => P[k]),
      cc: (s.cc ?? []).map((k) => P[k]),
