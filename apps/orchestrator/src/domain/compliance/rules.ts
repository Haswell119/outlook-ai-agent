...
/**
 * Upper bound on the text every policy pattern is run against.
 *
 * `BODY_LIMIT_BYTES` allows a 2 MB request, and each pattern scans subject +
 * body + every attachment's extracted text. Even a linear regex over 2 MB, times
 * a dozen rules, times the request rate, is enough to starve the event loop —
 * and sensitive-data markers that only appear after 200k characters are not a
 * realistic compliance case.
 */
export const MAX_SCANNED_CHARS = 200_000;
...
export function findSensitiveData(text: string, policy: Policy): Array<{ name: string; severity: RiskLevel; sample: string }> {
  ...
  for (const p of policy.sensitiveDataPatterns) {
    // `compilePattern` returns undefined for an invalid or backtracking-prone
    // pattern; `PolicyService.put` refuses to store one in the first place.
