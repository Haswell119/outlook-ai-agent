...
/** Registrable part of a domain without its TLD (longbow.ch → longbow). */
 ...
 * characters (longbow-finance.com, longbow.ch.evil.io …).
 */
export function isLookalikeDomain(domain: string, internalDomains: string[]): boolean {
  ...
  for (const internal of internalDomains) {
    ...
    if (il.length >= 4 && d.includes(`${i}.`)) return true; // longbow.ch.attacker.com
