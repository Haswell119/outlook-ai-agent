...
import type { AuditAggregates, AuditCursor, AuditRepository, AuditUserSummary } from "../../ports/repositories.js";
...
export class PgAuditRepository implements AuditRepository {
  ...
  private where(q: Omit<AuditQuery, "page" | "pageSize">): { sql: string; params: unknown[] } {
    ...
    if (q.source) add("details->>'source' = ?", q.source);
  ...
  /**
   * Keyset page for `GET /audit/export`: newest first, strictly after the
   * cursor. Keyset (not OFFSET) so exporting 2 years of audit stays O(page)
   * per query and stays correct while new events are being written.
   */
  async listPage(q: Omit<AuditQuery, "page" | "pageSize">, limit: number, cursor?: AuditCursor): Promise<AuditEvent[]> {
    ...
    let extra = "";
    if (cursor) {
      all.push(cursor.timestamp, cursor.id);
      extra = `${sql ? " AND" : " WHERE"} (ts, id) < ($${all.length - 1}::timestamptz, $${all.length}::uuid)`;
    }
    all.push(limit);
    const { rows } = await this.pool.query<Row>(`SELECT * FROM audit_events ${sql}${extra} ORDER BY ts DESC, id DESC LIMIT $${all.length}`, all);
    return rows.map(toEvent);
  }

  /** Retention purge (AUDIT_RETENTION_DAYS). Deletes in bounded batches. */
  async purgeOlderThan(beforeIso: string): Promise<number> {
    let total = 0;
    for (;;) {
      const r = await this.pool.query(`DELETE FROM audit_events WHERE id IN (SELECT id FROM audit_events WHERE ts < $1 LIMIT 5000)`, [beforeIso]);
  ...
  async countsByType(): Promise<Record<string, number>> {
    const { rows } = await this.pool.query<{ type: string; n: string }>(`SELECT type, count(*)::text AS n FROM audit_events GROUP BY type`);
