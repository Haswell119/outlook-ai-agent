...
export class MockLlmProvider implements LlmProvider {
  ...
  failing = false;
  calls = 0;
  /** Model name of the last call (set by the two-tier router). */
  ...
  async complete(req: LlmRequest): Promise<LlmCompletion> {
    ...
    // Echo back the routed model so tests can assert on two-tier routing.
    const model = req.model && req.model.trim() !== "" ? req.model : this.model;
    this.lastModel = model;
    return { text, model, usage: { promptTokens: Math.ceil(user.length / 4), completionTokens: Math.ceil(text.length / 4) } };
  }
...
function answer(useCase: LlmUseCase, prompt: string, lang: Language): unknown {
  switch (useCase) {
    ...
    case "daily_brief":
      return dailyBrief(prompt, lang);
    default:
      return { answer: lang === "fr" ? "Réponse simulée." : "Simulated answer.", confidence: 0.5 };
  }
}

/** Headline + bullets from the fact sheet in the prompt (no email bodies there). */
function dailyBrief(prompt: string, lang: Language) {
  const fr = lang === "fr";
  const facts = /### FACTS\n([\s\S]*?)### END FACTS/.exec(prompt)?.[1] ?? "";
  const priority = facts
    .split("\n")
    .filter((l) => l.startsWith("- ["))
    .slice(0, 4)
    .map((l) => l.replace(/^- /, "").trim());
  const counts = /New emails since the previous brief: (\d+) \(analysed: (\d+), awaiting a reply: (\d+), phishing suspected: (\d+)\)/.exec(facts);
  const total = counts?.[1] ?? "0";
  const awaiting = counts?.[3] ?? "0";
  const headline = priority[0]
    ? fr
      ? `À traiter en priorité : ${priority[0]}`
      : `Top priority: ${priority[0]}`
    : fr
      ? `${total} nouveaux emails, ${awaiting} en attente de réponse.`
      : `${total} new emails, ${awaiting} awaiting a reply.`;
  const highlights = [fr ? `${total} nouveaux emails, ${awaiting} en attente de réponse.` : `${total} new emails, ${awaiting} awaiting a reply.`, ...priority];
  return { headline: truncate(headline, 200), highlights: highlights.slice(0, 6), confidence: 0.72 };
}
