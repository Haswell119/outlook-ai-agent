...
export interface OpenAiCompatibleOptions {
  ...
  /** Texts per /embeddings HTTP call (EMBEDDING_BATCH_SIZE, default 64). */
...
export class OpenAiCompatibleProvider implements LlmProvider, EmbeddingProvider {
  ...
  async complete(req: LlmRequest): Promise<LlmCompletion> {
    ...
    // Two-tier routing: the queue/router sets `req.model` (LLM_FAST_MODEL or LLM_MODEL).
  ...
  /** Batched: at most `embeddingBatchSize` texts per HTTP call (default 64). */
  async embed(texts: string[]): Promise<number[][]> {
    if (!texts.length) return [];
    const size = Math.max(1, this.opts.embeddingBatchSize ?? 64);
    const out: number[][] = [];
    for (let i = 0; i < texts.length; i += size) {
      const batch = texts.slice(i, i + size);
      const data = (await this.post("/embeddings", { model: this.opts.embeddingModel, input: batch })) as { data?: Array<{ index?: number; embedding: number[] }> };
      const rows = (data.data ?? []).slice().sort((a, b) => (a.index ?? 0) - (b.index ?? 0));
      if (rows.length !== batch.length) throw new LlmError("output", `Embedding endpoint returned ${rows.length} vectors for ${batch.length} inputs`);
