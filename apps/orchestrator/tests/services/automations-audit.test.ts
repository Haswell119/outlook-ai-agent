...
describe("AutomationCoachService", () => {
  it("observe → detect (no duplicates) → simulate → approve; reject; edit rule", async () => {
    ...
    // The edit is audited too: a change to a live rule is a governance event
    // (the 4th entry), and it resets the rule to `proposed` pending a new simulation.
    expect(c.repos.audit.events.map((e) => e.type)).toEqual(["automation_proposed", "automation_simulated", "automation_approved", "automation_proposed", "automation_rejected"]);
    expect(c.repos.audit.events[3]).toMatchObject({ details: { change: "updated", fromStatus: "active", toStatus: "proposed", definitionChanged: true } });
  });
});
