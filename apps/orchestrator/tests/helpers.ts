...
export const user = (email = "dev.user@longbow.ch", roles: AuthenticatedUser["roles"] = ["user"], extra: Partial<AuthenticatedUser> = {}): AuthenticatedUser => ({ id: email, email, displayName: email.split("@")[0], roles, via: "dev-headers", ...extra });
...
export const sampleEmail = (over: Partial<EmailContext> = {}): EmailContext => ({
  ...
  to: [{ address: "dev.user@longbow.ch" }],
  cc: [],
  bcc: [],
