...
import type { GraphClient, GraphDeltaPage, MailboxAccess } from "../src/ports/graph.js";

/**
 * Scriptable Graph double for the sync state machine: delta pages are queued
 * and served one per call, exactly like Graph's `@odata.nextLink` /
 * `@odata.deltaLink` behaviour.
 */
export class FakeGraphClient implements GraphClient {
  readonly enabled = true;
  /** Pages served by `deltaInbox`, in order. */
  readonly pages: GraphDeltaPage[] = [];
  readonly deltaTokensSeen: Array<string | undefined> = [];
  readonly accessesSeen: MailboxAccess[] = [];
  groupMembers: string[] = [];
  /** When set, `deltaInbox` throws it once (simulates a 429 storm / revoked token). */
  failNextWith: Error | undefined;
  /** Home account id returned by `rememberDelegatedUser` (undefined = cannot remember). */
  homeAccountId: string | undefined = "home-account-1";
  accessible = true;

  queue(messages: EmailContext[], opts: { deltaToken?: string; hasMore?: boolean; removedIds?: string[] } = {}): this {
    this.pages.push({ messages, deltaToken: opts.deltaToken ?? `delta-${this.pages.length + 1}`, hasMore: opts.hasMore ?? false, removedIds: opts.removedIds ?? [] });
    return this;
  }

  async deltaInbox(access: MailboxAccess, deltaToken: string | undefined): Promise<GraphDeltaPage> {
    this.accessesSeen.push(access);
    this.deltaTokensSeen.push(deltaToken);
    if (this.failNextWith) {
      const e = this.failNextWith;
      this.failNextWith = undefined;
      throw e;
    }
    return this.pages.shift() ?? { messages: [], deltaToken, hasMore: false, removedIds: [] };
  }
  async rememberDelegatedUser() {
    return this.homeAccountId ? { homeAccountId: this.homeAccountId } : undefined;
  }
  async canAccess() {
    return this.accessible;
  }
  async listGroupMemberUpns() {
    return this.groupMembers;
  }

  /* Unused by the sync tests. */
  async getMessage(): Promise<EmailContext> {
    throw new Error("not implemented");
  }
  async getConversationMessages(): Promise<EmailContext[]> {
    return [];
  }
...
/** Environment that turns the precomputation worker on with a fake Graph. */
export const SYNC_ENV: NodeJS.ProcessEnv = {
  GRAPH_ENABLED: "true",
  PRECOMPUTE_ENABLED: "true",
  AAD_TENANT_ID: "tenant-1",
  AAD_CLIENT_ID: "client-1",
  AAD_CLIENT_SECRET: "secret-1",
  AUTH_MODE: "aad",
};
