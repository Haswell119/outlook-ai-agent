...
import { EscalationRequestSchema, SimulateAutomationRequestSchema, AutomationDecisionRequestSchema, DailyBriefRequestSchema } from "@oao/shared";
...
export interface OaoApi {
  ...
  /** `GET /config/features` — flags, model names, organisation name. */
  features(): Promise<FeatureFlags>;
  /**
   * `GET /analyze/email/:id` — precomputed or content-cached analysis.
  ...
  /** `GET /brief/daily` — stored brief; `null` when none exists for the day. */
  dailyBrief(req?: DailyBriefRequest): Promise<DailyBrief | null>;
  /** `POST /brief/daily` — generate (costs a model call). */
  generateDailyBrief(req?: DailyBriefRequest): Promise<DailyBrief>;
  /** `GET /mailbox/sync` — precomputation status. */
  mailboxSync(): Promise<MailboxSyncStatus>;
  /** `POST /mailbox/sync` — ask the worker to sync now. */
  syncNow(): Promise<MailboxSyncStatus>;
}
