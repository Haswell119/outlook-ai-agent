import { Button, makeStyles, Skeleton as FluentSkeleton, SkeletonItem, Text } from "@fluentui/react-components";
...
export function ErrorState({ error, onRetry, hint }: { error: unknown; onRetry?: () => void; hint?: ReactNode }) {
  const s = useStyles();
  const { t } = useI18n();
  ...
  return (
    <div className={s.center} role="alert">
      ...
      {onRetry && (
        <Button appearance="primary" size="small" onClick={onRetry}>
          {t("app.retry")}
        </Button>
      )}
    </div>
...
export function Skeleton({ cards = 3, label }: { cards?: number; label?: string }) {
  ...
  return (
    <FluentSkeleton aria-label={label ?? "loading"} className={s.skeleton} data-testid="skeleton">
      {label && <Text size={200} style={{ color: colors.textSecondary }}>{label}</Text>}
