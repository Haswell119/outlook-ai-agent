 ...
 */
import { safeExternalLink, type Language } from "@oao/shared";
 ...
 *
 * `webLink` is data that travelled from a mailbox through the orchestrator, so
 * the scheme is checked here too: `window.open("javascript:…")` runs script in
 * the task pane's origin in the WebViews Outlook embeds. The orchestrator also
 * sanitises it at ingestion — this is the second lock on the same door, and the
 * one that protects links that never went through the index.
 */
export function openMessage(emailId: string | undefined, webLink?: string): boolean {
  const safeLink = safeExternalLink(webLink);
  if (safeLink) {
